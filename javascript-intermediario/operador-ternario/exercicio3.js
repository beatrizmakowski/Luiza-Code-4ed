...
console.log(condicoesDoTempo);
condicoesDoTempo.forEach(condicaoDoTempo => {
    pessoa.acessorio = (condicaoDoTempo === 'ensolarado' ? 'óculos escuros' :
                        condicaoDoTempo === 'chuvoso'    ? 'guarda-chuva'   : 
                        condicaoDoTempo === 'frio'       ? 'casaco'         : 
                        'nenhum acessório'
                        );
    console.log(`Quando o tempo está ${condicaoDoTempo}, ${pessoa.nome} usa ${pessoa.acessorio}.`);
});