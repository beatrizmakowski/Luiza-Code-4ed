/**
 * Implemente uma função que executa uma busca linear sobre um array de um tipo
 ...
 * 
 * Solução implementada: busca linear sobre um array de 10 livros, buscando por
 * uma correspondência total com o nome do autor solicitado.
 */
 ...
 const linearSearchByAuthor = (books, author) => {
    for (const book of books) {
        if (book.author === author) {
            return book
        }
    }
    return 'No results. Double-check the spelling.'
 }

 author = 'Alan Moore'
 result = linearSearchByAuthor(books, author);
 console.log(result);